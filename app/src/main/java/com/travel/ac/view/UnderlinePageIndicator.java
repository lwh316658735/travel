...
public class UnderlinePageIndicator extends View implements PageIndicator
{
	...
	private String[] mTitles = new String[] {"测试", "测试", "测试", "测试" };
	...
	protected void onDraw(Canvas canvas)
	{
		...
		final float textleft = paddingLeft + pageWidth;
		final float right = left + pageWidth;
		final float top = getHeight() - getPaddingBottom() - mTabLineHeight;
		final float title_bottom = getHeight() - getPaddingBottom() - mTabLineHeight;
		...
		if (mTitles != null && mTitles.length > 0)
		{
			for (int i = 0; i < mTitles.length; i++)
			{
				...
				canvas.drawText(mTitles[i], (pageWidth * i) + pageWidth / 3 + 0.5f, (top + mTabLineHeight) / 2, mTextPaint);
			}
		}
