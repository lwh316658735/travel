...
public class MyTravelAdapter extends BaseViewHolderAdapter<MyTravelViewHolder>
{
	...
	protected void initItemView(MyTravelViewHolder baseViewHolder)
	{
		if (mPosition != 3)
		{
			baseViewHolder.setIvIco((ImageView) mView.findViewById(R.id.iv_ico));
			baseViewHolder.setTvItemText((TextView) mView.findViewById(R.id.tv_item_text));
		}
	}
	...
	protected void initItemData(MyTravelViewHolder baseViewHolder)
	{
		if (mPosition != 3)
		{
			...
			baseViewHolder.getTvItemText().setText(myTravelItemBean.getTextStr());
			baseViewHolder.getIvIco().setBackgroundResource(GetAssignedResultId.GetAssigned(myTravelItemBean.getIcoText(), "mipmap"));
		}
	}
